...
-- Adding criteria of visible_at to ensure that only jobs that are not scheduled / after NOW can be retried
SELECT TRUE INTO retry_disabled FROM job_queue q WHERE q.campaign_id = selected_campaign_id AND status <> 'LOGGED' AND q.visible_at <= clock_timestamp() LIMIT 1;
